...
//  ┌── 什么是"字节流" ──────────────────────────────────────────┐
//  │                                                              │
//  │  计算机存储的一切数据, 本质都是 0 和 1 的序列。               │
//  │  每 8 个 bit 编为一组, 称为一个 字节 (byte)。    │
//  │                                                              │
//  │  文件在磁盘上就是一串连续的字节。                             │
//  │  读文件 = 把这串字节从磁盘复制到内存 (Vec<u8>)。              │
//  │  写文件 = 把内存中的字节数组刷到磁盘。                        │
//  │                                                              │
//  │  所有高级格式都建立在字节流之上:                              │
//  │                                                              │
//  │    "Hello"  ──UTF-8编码──▶  [72, 101, 108, 108, 111]        │
//  │    {a:1}    ──JSON编码──▶  [123, 34, 97, 34, 58, 49, 125]  │
//  │    图片.png  ──二进制──▶  [137, 80, 78, 71, ...]            │
//  │                                                              │
//  │  Rust 中读写字节流的核心 trait:                               │
//  │    Read  — 从源头"拉"字节 (文件、网络、内存)                  │
//  │    Write — 向目标"推"字节 (文件、网络、内存)                  │
//  │                                                              │
//  │  所有权视角:                                                  │
//  │    读: OS 内核缓冲区 → Rust 堆内存 (Vec<u8>)                  │
//  │        数据所有权从 OS 转移到 Rust 程序                       │
//  │    写: Rust 堆内存 → OS 内核缓冲区 → 磁盘                     │
//  │        数据所有权仍在 Rust, OS 只拿到一份副本                 │
//  │                                                              │
//  └──────────────────────────────────────────────────────────────┘

/// 演示最底层: 文件读到的是 Vec<u8> (原始字节流)。
/// 所有权: fs::read 内部从 OS 读字节到堆内存 → Vec<u8> 所有权 move 给调用方。
fn demo_raw_bytes() {
